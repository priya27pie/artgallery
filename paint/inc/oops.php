...
function readAll($table){
 ...
 function trackjoin($table1,$table2,$col1){
        //select all data
   // select * from track inner join booking on track.book_id=booking.book_id group by track.book_id order by booking.book_id
        $query = "SELECT * FROM " . $table1. " inner join " . $table2. "  on ".$table1.".".$col1."=".$table2.".".$col1." group by $table1.$col1 order by ".$table2.".".$col1." desc";  
      // echo $query;
        $stmt = $this->conn->prepare($query);
        $stmt->execute();
